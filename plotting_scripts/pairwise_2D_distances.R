...
indir <- "/PopPUNK-mod/publication_figures/"
df <- read.csv(paste0(indir,"inter_species_comparisons/poppunk_js_distances_euclidean.tsv"), sep = "\t")
...
{
  # get NE data
  ne_dataframe <- read_excel(paste0(indir, "Bobay_Ochman_2018_Ne_estimates.xlsx"), sheet="updated_categories")
  ...
  ne_dataframe$Life_Style <- factor(ne_dataframe$Life_Style, levels = c("Single-host pathogen", "Multi-host pathogen", "Single-host commensal", "Multi-host commensal", "Free-living", "Unknown"))
}
...
# Compute convex hulls per Life_Style group for each embedding
hull_pca  <- embedding_df %>% group_by(Life_Style) %>% slice(chull(PCoA1, PCoA2))
hull_tsne <- embedding_df %>% group_by(Life_Style) %>% slice(chull(tSNE1, tSNE2))
hull_umap <- embedding_df %>% group_by(Life_Style) %>% slice(chull(UMAP1, UMAP2))
...
# colour by lifestyle
{
  ...
  # p
  # ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_linear_label.svg"), plot = p, width=10, height = 6)
  # ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_linear_label.png"), plot = p, width=10, height = 6)
  
  # colour by lifestyle, plot linear
  p.pca <- ggplot(embedding_df, aes(x = PCoA1, y = PCoA2, label = Species, color = Life_Style)) +
    geom_polygon(data = hull_pca, aes(fill = Life_Style), alpha = 0.2, color = NA, show.legend = FALSE) +
    geom_point(size = 3) +
    geom_text(size=2,nudge_y = 0.02) +
    theme_light() +
    ...
    theme(axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 12), legend.title = element_text(size = 16), legend.text = element_text(size = 14))
  p.pca
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_PCA_label.svg"), plot = p.pca, width=10, height = 6)
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_PCA_label.png"), plot = p.pca, width=10, height = 6)
  
  
  # colour by lifestyle, plot UMAP
  p.umap <- ggplot(embedding_df, aes(x = UMAP1, y = UMAP1, label = Species, color = Life_Style)) +
    geom_polygon(data = hull_umap, aes(fill = Life_Style), alpha = 0.2, color = NA, show.legend = FALSE) +
    geom_point(size = 3) +
    ...
    theme(axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 12), legend.title = element_text(size = 16), legend.text = element_text(size = 14))
  p.umap
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_UMAP.svg"), plot = p.umap, width=10, height = 6)
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_UMAP.png"), plot = p.umap, width=10, height = 6)

  # colour by lifestyle, plot tsne
  p.tsne <- ggplot(embedding_df, aes(x = tSNE1, y = tSNE2, label = Species, color = Life_Style)) +
    geom_polygon(data = hull_tsne, aes(fill = Life_Style), alpha = 0.2, color = NA, show.legend = FALSE) +
    geom_point(size = 3) +
    ...
    theme(axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 12), legend.title = element_text(size = 16), legend.text = element_text(size = 14))
  p.tsne
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_tSNE.svg"), plot = p.tsne, width=10, height = 6)
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_tSNE.png"), plot = p.tsne, width=10, height = 6)
  ...
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_tSNE_PCA.svg"), plot = merged_plots, width=10, height = 12)
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_tSNE_PCA.png"), plot = merged_plots, width=10, height = 12)
}
...
# colour by species
{
  ...
  p <- ggplot(embedding_df, aes(x = V1, y = V2, color = Species)) +
    ...
    theme(axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 12), legend.title = element_text(size = 16), legend.text = element_text(size = 14))
  p
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_linear_species.svg"), plot = p, width=10, height = 6)
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_linear_species.png"), plot = p, width=10, height = 6)
  ...
  p <- ggplot(embedding_df, aes(x = UMAP1, y = UMAP1, color = Species)) +
    ...
    theme(axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 12), legend.title = element_text(size = 16), legend.text = element_text(size = 14))
  p
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_UMAP.svg"), plot = p, width=10, height = 6)
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_UMAP.png"), plot = p, width=10, height = 6)
  ...
  p <- ggplot(embedding_df, aes(x = tSNE1, y = tSNE2, color = Specie)) +
    ...
    theme(axis.text.x = element_text(size = 18), axis.text.y = element_text(size = 18), axis.title=element_text(size=16,face="bold"), strip.text.x = element_text(size = 12), legend.title = element_text(size = 16), legend.text = element_text(size = 14))
  p
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_tSNE.svg"), plot = p, width=10, height = 6)
  ggsave(paste0(indir, "inter_species_comparisons/pairwise_embeddings_tSNE.png"), plot = p, width=10, height = 6)
}

...
# all
{
  indir_overall <- "BOLFI_gridsearch/"
  #run <- "param_step_selection_only"
  #run <- "param_step_competition_only_max_100"
  run <- "param_step_competition_only_max_1000"
  indir <- paste0(indir_overall, "data/", run, "/")
  df_paths <- c(Sys.glob(paste0(indir, "*_mcmc_posterior_samples.csv")))
  outpref <- paste0(indir_overall, "figures/", run, "/")

...
pangenome.file <- paste0(indir, "ppmod_real_input_ATB.txt")
gtdb.file <- paste0(indir, "gtdbtk.bac120.summary.tsv")
traits.file <- paste0(indir, "metadata_with_traits.tsv")
outpref <- paste0(indir, "/SPIRE_analysis_pangenome_")
...
# add traits
pangenome.df <- read.csv(pangenome.file, sep="\t", header = TRUE)
merged.df <- merge(meta, pangenome.df, by.x = "taxa", by.y = "name")

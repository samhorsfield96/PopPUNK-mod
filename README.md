...
1. First we require a fitted PopPUNK model. Either generate your own fit following the instructions [here](https://poppunk-docs.bacpop.org), or download a fit from the [Bacpop](https://bacpop.org/poppunk-databases/) website.
...
3. Now we need pangenome size estimates. These can come from your favourite pangenome clustering tool. Alternatively, you can run [WTBcluster](https://github.com/samhorsfield96/WTBcluster) on the genomes you used in Step 1, which will generate a file called `tokenised_genomes_0.txt` that can be passed to [analyse_pangenome.py](./scripts/analyse_pangenome.py), which will generate all the statistics you need to run PopPUNK-mod
...
pan_genes=[total pangenome size]
core_genes=[number of core genes]
avg_gene_freq=[average proportion of pangenome found in each genome]
